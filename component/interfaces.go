...
type Widget interface {
	...
	// [重要] このメソッドはスタイルのコピーを返しますが、内部のポインタフィールド（例: Background, Padding）は
	// 元のウィジェットのスタイルが指す参照と同じです。返されたStyle構造体のポインタフィールドが
	// 指す先の値（例: `style.Padding.Top = 10`）を変更すると、元のウィジェットのスタイルにも影響が及ぶため、
	// 絶対に避けるべきです。スタイル全体を安全に変更するにはSetStyle()を使用してください。
