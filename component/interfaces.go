...
type Widget interface {
	...
	//
	// [設計上の注意] このメソッドはスタイルのコピーを返します。ただし、これはパフォーマンスを考慮した「シャローコピー」です。
	// つまり、`style.Style` 構造体自体はコピーされますが、その内部のポインタフィールド（例: `Background`, `Padding`）は、
	// 元のウィジェットが持つポインタと同じアドレスを指します。
	// そのため、 `s := widget.GetStyle()` のように取得したスタイルに対し、`*s.Background = color.Red` のような操作を行うと、
	// 元のウィジェットの背景色も変更されてしまいます。
	// このような意図しない副作用を避け、ウィジェットの状態管理を明確にするため、
	// スタイルの変更は必ず `SetStyle()` メソッドを通して行うようにしてください。
