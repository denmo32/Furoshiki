...
func NewGame() *Game {
    ...
    // --- ヘッダーの作成 ---
    header, err := component.NewLabelBuilder().
    ...
    if err != nil {
        ...
        header, _ = component.NewLabelBuilder().Text("Header").Build()
    }

    // --- サイドバーの作成 ---
    // ボタンビルダーは自動で最小サイズを計算するため、CalculateMinSize()の呼び出しは不要
    sideButton1, err := component.NewButtonBuilder().Text("Dashboard").Flex(0).Size(0, 35).
    ...
    if err != nil {
        ...
        sideButton1, _ = component.NewButtonBuilder().Text("Button1").Build()
    }

    sideButton2, err := component.NewButtonBuilder().Text("Analytics").Flex(0).Size(0, 35).
        Style(style.Style{Font: mplusFont}).
        HoverStyle(style.Style{Background: color.RGBA{R: 200, G: 220, B: 255, A: 255}}).
    ...
    if err != nil {
        ...
        sideButton2, _ = component.NewButtonBuilder().Text("Button2").Build()
    }

    sideButton3, err := component.NewButtonBuilder().Text("Settings").Flex(0).Size(0, 35).
        Style(style.Style{Font: mplusFont}).
        HoverStyle(style.Style{Background: color.RGBA{R: 200, G: 220, B: 255, A: 255}}).
    ...
    if err != nil {
        ...
        sideButton3, _ = component.NewButtonBuilder().Text("Button3").Build()
    }
    ...
    // --- フッター (ステータスバー) の作成 ---
    statusLabel, err := component.NewLabelBuilder().Text("Status: OK").Style(style.Style{Font: mplusFontSmall}).Build()
    if err != nil {
        log.Printf("Error creating statusLabel: %v", err)
        statusLabel, _ = component.NewLabelBuilder().Text("Status").Build()
    }
    
    versionLabel, err := component.NewLabelBuilder().Text("Version 1.0.0").Style(style.Style{Font: mplusFontSmall}).Build()
    if err != nil {
        log.Printf("Error creating versionLabel: %v", err)
        versionLabel, _ = component.NewLabelBuilder().Text("Version").Build()
    }
...
func createMainContent() (*container.Container, error) {
    refreshButton, err := component.NewButtonBuilder().Text("Refresh").OnClick(func() { fmt.Println("Refresh!") }).Style(style.Style{Font: mplusFont}).Build()
    if err != nil {
        return nil, fmt.Errorf("error creating refreshButton: %w", err)
    }
    
    exportButton, err := component.NewButtonBuilder().Text("Export").OnClick(func() { fmt.Println("Export!") }).
...
func createCard(title, mainText, subText string) (component.Widget, error) {
    titleLabel, err := component.NewLabelBuilder().Text(title).Style(style.Style{Font: mplusFontSmall, TextColor: color.Gray{Y: 100}}).Build()
    if err != nil {
        return nil, fmt.Errorf("error creating titleLabel: %w", err)
    }
    
    mainLabel, err := component.NewLabelBuilder().Text(mainText).Style(style.Style{Font: mplusFont, TextColor: color.Black}).Size(0, 40).Build()
    if err != nil {
        return nil, fmt.Errorf("error creating mainLabel: %w", err)
    }
    
    subLabel, err := component.NewLabelBuilder().Text(subText).Style(style.Style{Font: mplusFontSmall, TextColor: color.Gray{Y: 120}}).Build()
    if err != nil {
